...
from topologies import configure_topology, FullyConnectedTopology
...
def train(
    ...
    elif config["algorithm"] == "relaysgd":
        yield from relaysgd(config, task, timer)
...
def relaysgd(config, task: Task, timer: Timer): # (task, world, learning_rate, num_steps):
    assert config["topology"] == "fully-connected"
    num_workers = config["distributed_world_size"]
    print(f"num_workers is {num_workers}")
    world = FullyConnectedTopology(num_workers)
    ...
    d = 2 
    state = torch.zeros(num_workers, d, 1) + 1
    parameters, _ = task.initialize(seed=config["seed"]) # state
    print("================ Parameters ==================")
    print(f"length of parameters are {len(parameters)}")
    ...
    for step, batch in task.data.iterator(
        ...
        # loss, g, state = task.loss_and_gradient(parameters, state[], batch)
        # print(f"The state after loss and gradient is {state}")
        ...
        for worker in world.workers:
            # print(f"state[worker] is {state[worker]}")
            loss, g, state_list = task.loss_and_gradient(parameters, state[worker], batch) 
            # print(f"state_list is {state_list}")
            # state[worker]
            yield (
            TrainStats(step, bytes_sent),
            BatchStats(loss=state[worker]),
            state[worker],
            state,
            )
        ...
        for worker in world.workers:
            print(f"The current worker is {worker}")
            print(f"The state is {state[worker]}")
            neighbors = world.neighbors(worker)
...
def allreduce(config, task: Task, timer: Timer):
    ...
    print(f"The state before loss and gradient is {state}")
