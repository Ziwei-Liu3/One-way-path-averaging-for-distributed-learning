...
config = dict(
    ...
    distributed_rank=0,
    distributed_world_size=3,  # 1 = turn off
...
def main():
    ...
    for train_stats, batch_stats, parameters, state in train(config, task, timer):
        ...
        if batch_stats.loss is not None:
            ...
            print(f"batch_stats.loss is {batch_stats.loss}")
...
def configure_task() -> Task:
    if config["task"] == "Cifar":
        ...
        print(f"os.getenv(DATA) is {data_path}")
...
def init_distributed_pytorch():
    if config["distributed_world_size"] > 1:
        ...
        #     )
        # else:
        if config["distributed_backend"] == "gloo":
            print("Initializing with Gloo")
