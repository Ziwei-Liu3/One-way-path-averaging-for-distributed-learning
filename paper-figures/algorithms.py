...
class QuadraticsTask:
    ...
    def error(self, state):
        ...
        print(f"The state is {state.squeeze()}")
        print(f"The target is {self.target.squeeze()}")
...
def pushsum(task, world, learning_rate, num_steps):
    ...
    for step in range(num_steps):
        ...
        for worker in world.workers:
            neighbors = world.neighbors(worker) # original 
            for neighbor in neighbors:
                new_messages[worker, neighbor] = state[worker]
        ...
        for worker in world.workers:
            ...
            num_messages = 0 
            message = 0 
            for neighbor in world.workers: # neighbors:
                # print(f"messages[neighbor, worker]: {messages[neighbor, worker]}")
                # print(f"messages[neighbor, worker] is {messages[neighbor, worker]}")
                if not isinstance(messages[neighbor, worker], float):
                    if (messages[neighbor, worker] != 0.0).any():
                        num_messages += 1 
                        message += messages[neighbor, worker]
                    
            state[worker] = (state[worker] * (world.num_workers - num_messages) + message) / world.num_workers

        num_messages_sent += world.max_degree
    yield Iterate(step, state, num_messages_sent)

def pushsum_with_comments(task, world, learning_rate, num_steps):
    state: torch.Tensor = task.init_state()  # shape [num_workers, ...]
    ...
    # push_sum_avg_steps = 1 # in cifar_doubletree.py 
    ...
    for step in range(num_steps):
        ...
        for worker in world.workers:
            neighbors = world.neighbors(worker) # original 
            send_buffer = torch.zeros_like(state[worker])
            # print(f"Send buffer is initialized to be {send_buffer}")
            # recv_buffers = {}
            ...
            for neighbor in neighbors:
                # for n in neighbors:
                #     if n != neighbor: 
                #         send_buffer += messages[n, worker]
                # send_buffer += state[worker]
                # send_buffer /= len(neighbors)
                # send_buffer /= len(neighbors)
                # print(f"Send buffer is updated to be {send_buffer}")
                
                # Send messages 
                send_buffer = state[worker]
                # messages[worker, neighbor] = state[worker]
                # Update messages and counts 
                new_messages[worker, neighbor] = send_buffer 
                # new_counts[worker, neighbor] = 1 + sum(counts[n, worker] for n in neighbors if n != neighbor)
                
                
                # Receive messages 
                # for neighbor in neighbors:
                # print(f"Send buffer is {send_buffer}")
                # recv_buffer = new_messages[neighbor, worker]
                # if random.uniform(0, 1) > simulated_dropped_message_probability: 
                #     # print(f"Send buffer is {send_buffer}")
                ...
                # recv_buffers[neighbor] = avg_buffer
        
        messages = new_messages
        
        for worker in world.workers:
            # Update messages and counts after the sending and receiving are done. 
            # for neighbor in neighbors:
            #     messages[worker, neighbor] = recv_buffers[neighbor]
            #     counts[worker, neighbor] = new_counts[worker, neighbor]
            
            # Apply averaging 
            # num_messages = sum(counts[n, worker] for n in neighbors)
            # state[worker] = (state[worker] * (world.num_workers - num_messages)
            #     + sum(messages[n, worker] for n in neighbors)) / world.num_workers
            neighbors = world.neighbors(worker)
            num_messages = 0 
            message = 0 
            for neighbor in neighbors:
                ...
                # state[worker] = (state[worker] * (world.num_workers - len(world.neighbors(worker)))
                #     + sum(messages[n, worker] for n in neighbors)) / world.num_workers
            state[worker] = (state[worker] * (world.num_workers - num_messages) + message) / world.num_workers
...
def update_state(weight_vector, state, path):
    path_workers = len(path)
    # print(f"The length of path is {path_workers}")
    # print(f"The path is {path}")
    W = torch.zeros((path_workers, path_workers))
    # print(f"The shape of W is {W.shape}")
    # print(f"The weight_vector is {weight_vector.squeeze()}")
    w_vector_path = weight_vector[path, :]
    # print(f"The w_vector_path is {w_vector_path.squeeze()}")
    
    # Update weight vector 
    w_vector_new = torch.zeros(path_workers, 1)
    assert w_vector_path.shape[0] == path_workers, "weight_vector.shape[0] should be equal to path_workers"
    for j in range(path_workers):
        for k in range(j + 1):
            w_vector_new[j] += (1 / (path_workers - k)) * w_vector_path[k]
    
    # Update the state 
    state_path = state[path, :]
    # print(f"The shape of state_path is {state_path.shape}")
    for j in range(path_workers):
        denom = 0 
        for k in range(j + 1):
            denom += (w_vector_new[k] / (path_workers - k)).item()
        for k in range(j + 1):
            W[j, k] = (w_vector_new[k] / (path_workers - k)) / (denom)
    ones = torch.ones(path_workers)
    assert ((torch.matmul(W, ones) - ones) < 1e-5).all()
    state_path_new = torch.matmul(W, state_path.squeeze()).unsqueeze(2)
    
    return w_vector_new, state_path_new

def choose_random_be(total_workers):
    import random
    beginning = random.randrange(0,total_workers-1)
    end = random.randrange(0,total_workers-1)
    while beginning == end: 
        beginning = random.randrange(0,total_workers-1)
        end = random.randrange(0,total_workers-1)
    return beginning, end

def find_random_path(total_workers, topology, ring = False):
    beginning, end = choose_random_be(total_workers)
    path = [beginning]
    current = beginning 
    neighbor = beginning
    if ring == False: 
        while neighbor != end: 
            ...
            # print(f"The found neighbor is {neighbor}")
            # To avoid loops 
            # while (neighbor in path) and (not (len(topology.neighbors(neighbor)) == 1 and topology.neighbors(neighbor)[0] != end)): 
            #     neighbor = random.choice(neighbors)
            #     print(f"The found neighbor is {neighbor}")
            if neighbor in path:
                path = path[:path.index(neighbor)]
...
def one_way_path_averaging(task, world, learning_rate, num_steps, overlap=False, num_communication_rounds=16):
    state = task.init_state()

    # print(f"num_communication_rounds = {num_communication_rounds}")

    num_messages_sent = 0
    total_workers = len(world.workers)
    w_vector = torch.ones(total_workers, 1)
    i = 0 
    for step in range(num_steps):
        # if step % 100 == 0:
            # print(f"The step is at {step}")
        yield Iterate(step, state, num_messages_sent)
        g = task.grad(state)
        ...
        for _ in range(num_communication_rounds):
            path = find_random_path(total_workers, world)
            # if num_steps % 100 == 0:
                # print(f"The path is {path}")
            # sys.exit()
            w_vector_new, state_path = update_state(w_vector, state, path)
            state[path, :] = state_path
            w_vector[path, :] = w_vector_new
            i = i + 1 
